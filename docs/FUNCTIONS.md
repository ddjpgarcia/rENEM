...
| `enem_trend(var, years)` | Serie temporal ponderada de una variable del **panel armonizado** (`mujer`, `edad_grupo`, ISCO-08, `pdte_acuerdo`/`pdte_aprueba`, `ideologia_lr`, `eval_pan`/`eval_prd`/`eval_pri`) | Funcional (necesita `survey`) | [`enem_load()`](https://ddjpgarcia.github.io/rENEM/reference/enem_load.md) |
